...
describe("ProgressBar", () => {
	...
	test("sets aria-valuemin and aria-valuemax", () => {
		render(ProgressBar, { props: { value: 50, max: 200 } });
	...
	test("uses the label prop as aria-label", () => {
		render(ProgressBar, { props: { value: 50, label: "Upload progress" } });
		expect(screen.getByRole("progressbar")).toHaveAttribute(
			"aria-label",
			"Upload progress",
		);
	...
	test("renders the label text when provided", () => {
		render(ProgressBar, { props: { value: 50, label: "Uploading..." } });
	...
	test("shows the percentage value when showValue is true", () => {
		render(ProgressBar, { props: { value: 60, showValue: true } });
