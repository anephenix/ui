...
describe('DesktopMenu', () => {
	...
	test('renders without crashing', () => {
		const { container } = render(<DesktopMenu links={links} loggedIn={true} Link={Link} />);
		expect(container).toBeInTheDocument();
	});

	test('renders only links that are not hidden on desktop and pass hideOptions', () => {
		const { getAllByText } = render(<DesktopMenu links={links} loggedIn={true} Link={Link} />);
	...
	test('does not render links that are hidden on desktop', () => {
		const { queryByText } = render(<DesktopMenu links={links} loggedIn={true} Link={Link} />);
